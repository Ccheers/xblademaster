...
func (engine *Engine) LoadHTMLGlob(pattern string) (err error) {
	if engine.loadHtml == 1 {
		return errors.New("已经加载过html，不能重复加载")
	}
...
func (engine *Engine) LoadHTMLFiles(files ...string) (err error) {
	if engine.loadHtml == 1 {
		return errors.New("已经加载过html，不能重复加载")
	}
	if err = engine.LoadHTMLFiles(files...); err != nil {
		return
	}
	engine.loadHtml = 1
	return
